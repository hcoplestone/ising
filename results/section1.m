...
for beta = betaLowerLim:betaStep:betaUpperLim
    ...
    meanMagnetisations = mean(Magnetisations, 2)
    standDevInMagnetisations = std(Magnetisations, 0, 2);
%     plot(Sweeps(:,1), meanMagnetisations)
%     errorbar(Sweeps(:,1), meanMagnetisations, standDevInMagnetisations, 'LineStyle', 'none')
    errorbar(Sweeps(:,1), meanMagnetisations, standDevInMagnetisations, 'x-', 'DisplayName', sprintf('$\\beta = %0.2f$', beta/100))
